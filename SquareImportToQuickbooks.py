...
def process_order(sr):
    ...
    if customer_record.get('QueryResponse') and customer_record['QueryResponse']['Customer']:
        ...
        print("Need to create a customer here")
...
key_list = ['black', 'red', 'hardwood', 'brown', 'spread', 'donxxate', 'donxxation']
...
for order in all_orders['orders']:
    ...
    if parse(order.get('created_at')).date() >= parse('2021-02-07T22:12:39.101Z').date() and order.get('customer_id') == 'AXC7568MTX2SZ2CRY448RWD61C':
        #sr = SalesReceipt()
            ...
            for item in order['line_items']:
                ...
                if 'name' in item:
                    ...
                    if any(regex.match(item['name'].lower()) for regex in reg_list):
                        ...
                        if bool(item.get('customer_id', None)):
                            ...
                            #print("getting from fufillments")
